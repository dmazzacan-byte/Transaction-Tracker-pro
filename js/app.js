...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    onAuthStateChanged(auth, user => {
        if (user) {
            ...
            listenersAttached = false; // Reset on logout
        }
    });
    ...
    async function initApp() {
        ...
        // Setup essential listeners that only need to be attached once.
        ...
        setupFilters();
        renderAll();
        setupDashboard(); // Setup dashboard before attaching its specific listeners

        document.body.dataset.ready = 'true'; // Signal that the app is ready
    }
    async function fetchData() {
        ...
        customers = cSnap.docs.map(doc => ({ id: doc.id, ...doc.data() })); orders = oSnap.docs.map(doc => ({ id: doc.id, ...doc.data() })); payments = paySnap.docs.map(doc => ({ id: doc.id, ...doc.data() }));
    ...
    function renderOrders() {
        ...
        const filtered = orders.filter(o => new Date(o.date).getMonth() === month && new Date(o.date).getFullYear() === year && (!customerId || o.customerId === customerId) && (!productId || (Array.isArray(o.items) && o.items.some(item => item.productId === productId))) && (!status || o.status === status));
        filtered.sort((a, b) => new Date(b.date) - new Date(a.date));
        DOMElements.ordersTableBody.innerHTML = filtered.map(o => { const c = customers.find(c => c.id === o.customerId)?.name || 'N/A'; const itemsHtml = Array.isArray(o.items) ? o.items.map(item => `${item.quantity} x ${products.find(p => p.id === item.productId)?.description || 'N/A'}`).join('<br>') : (o.items || ''); return `<tr><td>${new Date(o.date).toLocaleDateString()}</td><td>${c}</td><td>${itemsHtml}</td><td>${Array.isArray(o.items) ? o.items.reduce((sum, i) => sum + i.quantity, 0) : ''}</td><td>$${o.total.toFixed(2)}</td><td>$${(o.amountPaid||0).toFixed(2)}</td><td><span class="status status-${o.status.toLowerCase()}">${t(`status${o.status}`)}</span></td><td><button class="action-btn edit" data-id="${o.id}" data-type="order" title="${t('editOrderTitle')}"><i class="fas fa-edit"></i></button><button class="action-btn delete" data-id="${o.id}" data-type="order" title="${t('actions')}"><i class="fas fa-trash"></i></button><button class="action-btn pay" data-id="${o.id}" data-type="order" title="${t('registerPaymentTitle')}"><i class="fas fa-dollar-sign"></i></button></td></tr>`; }).join('');
    }
    ...
    function setupEventListeners() {
        ...
        DOMElements.addPaymentBtn.addEventListener('click', () => {
            DOMElements.paymentForm.reset();
            const pendingOrders = orders.filter(o => o.status !== 'Paid');
            const options = pendingOrders.map(o => {
                const customer = customers.find(c => c.id === o.customerId);
                return {
                    id: o.id,
                    text: `${customer?.name || 'N/A'} - ${new Date(o.date).toLocaleDateString()} - $${o.total.toFixed(2)}`
                };
            });
            populateSelect('payment-order', options, 'id', 'text', null, t('selectOrder'));
            document.getElementById('payment-modal-title').textContent = t('newPayment');
            openModal('payment-modal');
        });
        DOMElements.addOrderItemBtn.addEventListener('click', addOrderItemRow);
        DOMElements.orderItemsContainer.addEventListener('change', e => { if (e.target.classList.contains('order-item-product') || e.target.classList.contains('order-item-quantity') || e.target.classList.contains('order-item-price-type') || e.target.classList.contains('order-item-discount')) { updateOrderTotal(); } });
        DOMElements.orderStatus.addEventListener('change', () => {
            const isPayment = DOMElements.orderStatus.value === 'Paid' || DOMElements.orderStatus.value === 'Partial';
            DOMElements.paymentDetailsContainer.classList.toggle('hidden', !isPayment);
        });

        DOMElements.productForm.addEventListener('submit', e => { e.preventDefault(); handleSave('products', DOMElements.productForm.querySelector('#product-id').value, { description: DOMElements.productForm.querySelector('#product-description').value, retailPrice: parseFloat(DOMElements.productForm.querySelector('#product-retail-price').value), wholesalePrice: parseFloat(DOMElements.productForm.querySelector('#product-wholesale-price').value) }); });
        DOMElements.customerForm.addEventListener('submit', e => { e.preventDefault(); const id = DOMElements.customerForm.querySelector('#customer-id').value; const data = { name: DOMElements.customerForm.querySelector('#customer-name').value, phone: DOMElements.customerForm.querySelector('#customer-phone').value }; handleSave('customers', id, data); });
        DOMElements.orderForm.addEventListener('submit', async e => {
            e.preventDefault();
            const items = getOrderItemData();
            const total = calculateOrderTotal(items);
            const status = DOMElements.orderStatus.value;
            const amountPaid = parseFloat(document.getElementById('order-amount-paid').value) || 0;
            const reference = document.getElementById('order-reference').value;
            ...
            const orderData = {
                ...
                total,
                status,
                amountPaid: status === 'Paid' ? total : (status === 'Partial' ? amountPaid : 0),
                date: new Date().toISOString()
            };

            const orderId = await saveOrUpdate('orders', DOMElements.orderForm.querySelector('#order-id').value, orderData, true);

            if ((status === 'Paid' || status === 'Partial') && orderData.amountPaid > 0) {
    ...
    async function handleSave(collectionName, id, data) {
        const newId = await saveOrUpdate(collectionName, id, data, true);
        if (collectionName === 'customers' && !id) {
            if (document.getElementById('customer-modal').dataset.fromOrderModal === 'true') {
                document.getElementById('order-customer-autocomplete').value = data.name;
                document.getElementById('order-customer-id').value = newId;
            }
        }
        closeModal();
        await fetchData(); // Just refetch and rerender
        renderAll();
    }
    async function saveOrUpdate(coll, id, data, returnId = false) {
    ...
    function addOrderItemRow(item = {}) {
        ...
        const productOptions = products.map(p => `<option value="${p.id}" ${item.productId === p.id ? 'selected' : ''}>${p.description}</option>`).join('');
        row.innerHTML = `
            <select class="order-item-product"><option value="" disabled selected>${t('selectProduct')}</option>${productOptions}</select>
            <input type="number" class="order-item-quantity" value="${item.quantity || 1}" min="1">
            <select class="order-item-price-type">
                <option value="retail" ${item.priceType === 'retail' ? 'selected' : ''}>${t('priceRetail')}</option>
                <option value="wholesale" ${item.priceType === 'wholesale' ? 'selected' : ''}>${t('priceWholesale')}</option>
            </select>
            <input type="number" class="order-item-discount" placeholder="${t('discount')}" value="${item.discount || 0}" min="0">
            <button type="button" class="remove-order-item-btn"><i class="fas fa-trash"></i></button>`;
    ...
    function getOrderItemData() { const items = []; DOMElements.orderItemsContainer.querySelectorAll('.order-item-row').forEach(row => { const productId = row.querySelector('.order-item-product').value; const quantity = parseInt(row.querySelector('.order-item-quantity').value); const priceType = row.querySelector('.order-item-price-type').value; const discount = parseFloat(row.querySelector('.order-item-discount').value) || 0; if (productId && quantity > 0) { items.push({ productId, quantity, priceType, discount }); } }); return items; }
    function calculateOrderTotal(items) { return items.reduce((total, item) => { const product = products.find(p => p.id === item.productId); if (!product) return total; const price = item.priceType === 'wholesale' ? product.wholesalePrice : product.retailPrice; return total + (price * item.quantity) - item.discount; }, 0); }
    ...
    DOMElements.backupBtn.addEventListener('click', () => {
        ...
        const prodMap = new Map(products.map(p => [p.id, p.description]));
        const ordersExport = orders.map(({ customerId, id, items, ...rest }) => ({
            customer: custMap.get(customerId) || 'N/A',
            items: items.map(item => `${item.quantity} x ${prodMap.get(item.productId)}`).join(', '),
            ...rest
        }));
        const toSheet = data => XLSX.utils.json_to_sheet(data.map(({id, ...rest}) => rest));
        XLSX.utils.book_append_sheet(wb, toSheet(products), "Products");
    ...
    function setupDashboard() {
        // Ensure listeners are only attached once if this function were ever called multiple times.
        if (!DOMElements.dashboardMonthSelect.dataset.listener) {
            DOMElements.dashboardMonthSelect.addEventListener('change', updateDashboard);
            DOMElements.dashboardYearSelect.addEventListener('change', updateDashboard);
            DOMElements.dashboardMonthSelect.dataset.listener = 'true';
        }
        updateDashboard();
    }
