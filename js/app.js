...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function setupEventListeners() {
        ...
        DOMElements.orderForm.addEventListener('submit', e => { e.preventDefault(); const items = getOrderItemData(); const total = calculateOrderTotal(items); const status = DOMElements.orderForm.querySelector('#order-status').value; handleSave('orders', DOMElements.orderForm.querySelector('#order-id').value, { customerId: document.getElementById('order-customer-id').value, items, total, status, amountPaid: status === 'Paid' ? total : 0, date: new Date().toISOString() }); });
    ...
    async function handleSave(collectionName, id, data) {
        await saveOrUpdate(collectionName, id, data);
        closeModal();
        await fetchData(); // Just refetch and rerender
        renderAll();
    }
    async function saveOrUpdate(coll, id, data) { const ref = collection(db, `users/${currentUser.uid}/${coll}`); if (id) { await updateDoc(doc(ref, id), data); } else { await addDoc(ref, data); } }
