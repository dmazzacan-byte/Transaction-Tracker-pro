...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function initApp() { if (!currentUser) return; await fetchData(); setupFilters(); renderAll(); setupDashboard(); setupEventListeners(); }
    ...
    function renderOrders() {
        ...
        const filtered = orders.filter(o => new Date(o.date).getMonth() === month && new Date(o.date).getFullYear() === year && (!customerId || o.customerId === customerId) && (!productId || o.productId === productId) && (!status || o.status === status));
        filtered.sort((a, b) => new Date(b.date) - new Date(a.date));
        DOMElements.ordersTableBody.innerHTML = filtered.map(o => { const c = customers.find(c => c.id === o.customerId)?.name || 'N/A'; const p = products.find(p => p.id === o.productId)?.description || 'N/A'; return `<tr><td>${new Date(o.date).toLocaleDateString()}</td><td>${c}</td><td>${p}</td><td>${o.quantity}</td><td>$${o.total.toFixed(2)}</td><td>$${(o.amountPaid||0).toFixed(2)}</td><td><span class="status status-${o.status.toLowerCase()}">${t(`status${o.status}`)}</span></td><td><button class="action-btn edit" data-id="${o.id}" data-type="order" title="${t('editOrderTitle')}"><i class="fas fa-edit"></i></button><button class="action-btn delete" data-id="${o.id}" data-type="order" title="${t('actions')}"><i class="fas fa-trash"></i></button><button class="action-btn pay" data-id="${o.id}" data-type="order" title="${t('registerPaymentTitle')}"><i class="fas fa-dollar-sign"></i></button></td></tr>`; }).join('');
    }
    function renderPayments() {
        const customerId = DOMElements.paymentCustomerFilter.value;
        const filtered = payments.filter(p => { const order = orders.find(o => o.id === p.orderId); return !customerId || (order && order.customerId === customerId); });
        filtered.sort((a, b) => new Date(b.date) - new Date(a.date));
        DOMElements.paymentsTableBody.innerHTML = filtered.map(p => { const o = orders.find(o => o.id === p.orderId); const c = customers.find(c => c.id === o?.customerId)?.name || 'N/A'; return `<tr><td>${new Date(p.date).toLocaleDateString()}</td><td>${c}</td><td>${t('order')} #${o?.id.substring(0,5)}...</td><td>$${p.amount.toFixed(2)}</td><td>${p.reference||''}</td></tr>`; }).join('');
    }
    ...
    function setupEventListeners() {
        ...
        DOMElements.addOrderBtn.addEventListener('click', () => { DOMElements.orderForm.reset(); DOMElements.orderForm.querySelector('#order-id').value = ''; document.getElementById('order-customer-autocomplete').value = ''; document.getElementById('order-customer-id').value = ''; populateSelect('order-product', products, 'id', 'description'); document.getElementById('order-modal-title').textContent = t('newOrder'); openModal('order-modal'); });
        ...
        DOMElements.orderForm.addEventListener('submit', async e => { e.preventDefault(); const prod = products.find(p => p.id === DOMElements.orderForm.querySelector('#order-product').value); const qty = parseInt(DOMElements.orderForm.querySelector('#order-quantity').value); const stat = DOMElements.orderForm.querySelector('#order-status').value; const total = prod.retailPrice * qty; await saveOrUpdate('orders', DOMElements.orderForm.querySelector('#order-id').value, { customerId: document.getElementById('order-customer-id').value, productId: prod.id, quantity: qty, total, status: stat, amountPaid: stat === 'Paid' ? total : 0, date: new Date().toISOString() }); closeModal(); await initApp(); });
    ...
    function setupFilters() {
        populateDateFilters(DOMElements.dashboardMonthSelect, DOMElements.dashboardYearSelect);
        populateDateFilters(DOMElements.ordersMonthSelect, DOMElements.ordersYearSelect);
        populateSelect(DOMElements.orderCustomerFilter, customers, 'id', 'name', '', t('allCustomers'));
        populateSelect(DOMElements.orderProductFilter, products, 'id', 'description', '', t('allProducts'));
        populateSelect(DOMElements.paymentCustomerFilter, customers, 'id', 'name', '', t('allCustomers'));

        [DOMElements.ordersMonthSelect, DOMElements.ordersYearSelect, DOMElements.orderCustomerFilter, DOMElements.orderProductFilter, DOMElements.orderStatusFilter].forEach(el => el.addEventListener('change', renderOrders));
        DOMElements.paymentCustomerFilter.addEventListener('change', renderPayments);
    }
    ...
    // --- Dashboard ---
    function setupDashboard() { updateDashboard(); DOMElements.dashboardMonthSelect.addEventListener('change', updateDashboard); DOMElements.dashboardYearSelect.addEventListener('change', updateDashboard); }
    ...
    function renderProductSalesChart(data) { const sales = {}; data.forEach(o => { const name = products.find(p => p.id === o.productId)?.description || 'Unk'; sales[name] = (sales[name] || 0) + o.total; }); const labels = Object.keys(sales), values = Object.values(sales); const ctx = document.getElementById('product-sales-chart').getContext('2d'); if (productSalesChart) productSalesChart.destroy(); productSalesChart = new Chart(ctx, { type: 'pie', data: { labels, datasets: [{ data: values, backgroundColor: ['#007bff', '#28a745', '#ffc107', '#dc3545', '#17a2b8', '#6c757d'] }] }, options: { responsive: true } }); }
