...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function setupEventListeners() {
        ...
        DOMElements.orderForm.addEventListener('submit', async e => { e.preventDefault(); const items = []; DOMElements.orderItemsContainer.querySelectorAll('.order-item-row').forEach(row => { const productId = row.querySelector('.order-item-product').value; const quantity = parseFloat(row.querySelector('.order-item-quantity').value); if (productId && quantity > 0) { items.push({ productId, quantity }); } }); const total = calculateOrderTotal(items); const status = DOMElements.orderForm.querySelector('#order-status').value; await saveOrUpdate('orders', DOMElements.orderForm.querySelector('#order-id').value, { customerId: document.getElementById('order-customer-id').value, items, total, status, amountPaid: status === 'Paid' ? total : 0, date: new Date().toISOString() }); closeModal(); await initApp(); });
    ...
    function addOrderItemRow(item = null) {
        ...
        row.innerHTML = `<select class="order-item-product"><option value="">${t('selectOption')}</option>${productOptions}</select> <input type="number" class="order-item-quantity" value="${item ? item.quantity : 1}" min="1" step="0.01"> <button type="button" class="remove-order-item-btn"><i class="fas fa-trash"></i></button>`;
    ...
    function getOrderItemData() { const items = []; DOMElements.orderItemsContainer.querySelectorAll('.order-item-row').forEach(row => { const productId = row.querySelector('.order-item-product').value; const quantity = parseFloat(row.querySelector('.order-item-quantity').value); if (productId && quantity > 0) { items.push({ productId, quantity }); } }); return items; }
