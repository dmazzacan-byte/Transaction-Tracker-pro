...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function setupEventListeners() {
        ...
        DOMElements.orderForm.addEventListener('submit', async e => { e.preventDefault(); const items = getOrderItemData(); const total = calculateOrderTotal(items); const status = DOMElements.orderForm.querySelector('#order-status').value; await saveOrUpdate('orders', DOMElements.orderForm.querySelector('#order-id').value, { customerId: document.getElementById('order-customer-id').value, items, total, status, amountPaid: status === 'Paid' ? total : 0, date: new Date().toISOString() }); closeModal(); await initApp(); });
